  ...
  return (
    <section id="projects" className="py-12 sm:py-16 md:py-20 bg-gray-800">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <h2 className="text-2xl sm:text-3xl font-bold mb-8 sm:mb-12 text-center text-gray-100">Project experience</h2>
        <div className="grid gap-4 sm:gap-6 grid-cols-1 md:grid-cols-2">
          {projects.map((p) => (
            <article key={p.id} className="p-4 sm:p-6 border border-gray-700 rounded-lg shadow-sm hover:shadow-lg transition-all bg-gray-900/50 hover:bg-gray-900 hover:border-green-400">
              <div className="flex items-start justify-between mb-2 sm:mb-3">
                <div>
                  <h3 className="text-lg sm:text-xl font-semibold text-green-300">{p.title}</h3>
                  <p className="text-xs sm:text-sm text-gray-400">{p.period}</p>
                </div>
              </div>
              <p className="text-sm sm:text-base text-gray-300 mt-3 sm:mt-4 leading-relaxed">{p.desc}</p>
              <p className="text-xs sm:text-sm text-gray-400 mt-2 sm:mt-3"><strong className="text-green-200">Tech:</strong> {p.tech}</p>
              <a
                className="mt-3 sm:mt-4 inline-flex items-center gap-2 text-blue-400 hover:text-green-300 transition-colors font-medium text-sm"
                href={p.link}
                target="_blank"
